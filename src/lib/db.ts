...
function buildDemoPayload() {
  ...
  const transactions: Transaction[] = [
    ...
    { id: "tx-11", date: txDates[2], type: "expense", amount: 3560, currency: "DOP", usdAmount: 60.34, category: "Food & Dining", tags: ["travel"], paymentMethod: "cash", accountId: "acc-3", description: "Santo Domingo lunch", isRecurring: false, createdAt: now },
  ];
  ...
  const gymSessions: GymSession[] = [
    { id: "gym-1", date: today, name: "Upper Body Push", isCardio: false, notes: "Strong bench day", createdAt: now },
    { id: "gym-2", date: txDates[3], name: "Treadmill Run", isCardio: true, cardioType: "run", duration: 28, distance: 2.8, createdAt: now },
    { id: "gym-3", date: txDates[6], name: "Bike Intervals", isCardio: true, cardioType: "bike", duration: 22, distance: 7.4, createdAt: now },
  ];
  ...
  const settings: Settings = {
    dopRate: 59.5,
    dopRateUpdatedAt: now,
    primaryCurrency: "USD",
    theme: "light",
    zodiacTheme: "gemini",
    dashboardCardOrder: ["summary", "money", "health", "life", "weather", "transactions", "bills"],
    dashboardCardSizes: {
      summary: "large",
      weather: "medium",
      money: "medium",
...
export async function seedIfEmpty(): Promise<void> {
  ...
  await importAllData(JSON.stringify(demo));
}

export function createDemoSnapshot(): string {
  return JSON.stringify(buildDemoPayload());
}
...
export async function exportAllData(): Promise<string> {
  const [
    ...
    tasks, notes, quickLinks, houseEntries, contacts, gymSessions, settings,
  ] = await Promise.all([
  ...
  return JSON.stringify(
    { transactions, bills, debts, accounts, healthLogs, tasks, notes, quickLinks, houseEntries, contacts, gymSessions, settings, exportedAt: new Date().toISOString() },
    null,
    2
  );
}
...
export async function importAllData(json: string): Promise<void> {
  ...
  await db.transaction("rw", [db.transactions, db.bills, db.debts, db.accounts, db.healthLogs, db.tasks, db.notes, db.quickLinks, db.houseEntries, db.contacts, db.gymSessions], async () => {
    ...
    if (data.gymSessions) { await db.gymSessions.clear(); await db.gymSessions.bulkAdd(data.gymSessions); }
  });
  if (data.settings) {
